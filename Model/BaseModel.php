...
class BaseModel 
{
    ...
    {
     	$this->connect = new PDO("mysql:host=localhost; dbname=Project_SP;charset=utf8", 'root', '');
    }
    ...
    public static function view(){
        $model = new static();
        $sql = "SELECT sanpham.id, sanpham.ten_sp, nsx.ten_nsx, sanpham.hinhanh FROM sanpham INNER JOIN nsx ON sanpham.ma_nsx= nsx.ma_nsx";
        $stmt = $model->connect->prepare($sql);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_CLASS, get_class($model));
        return $result;
